...
# Load dataset
df = pd.read_csv("UK_Air_Quality_Cleaned.csv")
...
def update_dashboard(site, pollutant):
    ...
    # 🔥 Filter by site FIRST (huge performance boost)
    ...
    # -------------------------------
    # Monthly (use filtered, not full df)
    # -------------------------------
    ...
    # -------------------------------
    # Distribution (use filtered only)
    # -------------------------------
    dist = px.histogram(
        filtered,
        x=pollutant,
    ...
    # -------------------------------
    # Correlation (sample only)
    # -------------------------------
    corr = df[pollutants].sample(5000).corr()
